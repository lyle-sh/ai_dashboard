...
interface CalEvent { id:number; time:string; title:string; detail:string; color:string }
...
export default function Dashboard() {
  ...
  function saveCalEvent(){if(!ef.title.trim())return;if(editId)setCalEvents(p=>p.map(x=>x.id===editId?{...x,...ef}:x));else setCalEvents(p=>[...p,{id:Date.now(),...ef}]);closeModal();}
            ...
            <div className="card">
              ...
              {sortedCal.map(e=>(
                <div className="cal-item" key={e.id} onClick={()=>openModal('cal',e.id)}>
                  ...
                  <button className="xbtn" onClick={ev=>{ev.stopPropagation();setCalEvents(x=>x.filter(i=>i.id!==e.id))}}>✕</button>
                </div>
              ))}
            </div>
